...
| Classification | Use | Rule |
| --- | --- | --- |
| Observed | User supplied record, meeting note, or program artifact. | State whose observation it is. Do not imply external verification. |
| Source backed | Claim supported by a dated primary or authoritative source. | Keep URL, date, source title, and claim ID in the ledger. |
| Teaching synthesis | Original interpretation that connects evidence, a case, and a mechanism. | Label internally as synthesis. Do not present it as a measured fact. |
...
- Prefer a cited research paper or official method over a social post.
...
Do not freeze the session until every factual claim is source backed, every composite is marked internally, every synthesis has an evidence trail, and the one sustained case has a defensible source status.
