...
- Reject non-positive, fractional, or one-blob timing plans. Independently of the author-selected profile, require at least two core segments and one distinct segment per started thirty minutes of official time; the selected profile may set a higher floor. Verify every core segment states its facilitator move, participant move, artifact state, and recovery move. Verify every reserve states its trigger, added move, participant move, artifact state, and rejoin line.
